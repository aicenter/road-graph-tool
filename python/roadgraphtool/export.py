...
def get_map_edges_from_db(srid: int, area_id: int, schema='public') -> gpd.GeoDataFrame:
    ...
    sql = f"""
        ...
        SELECT
            ...
            "to" AS db_id_to,
            edges.geom as geom,
            st_length(st_transform(edges.geom, {srid})) as length,
            speed
        FROM edges
            JOIN demand_nodes from_nodes ON edges."from" = from_nodes.db_id
            JOIN demand_nodes to_nodes ON edges."to" = to_nodes.db_id
        WHERE
            edges.area = {area_id}::smallint -- This is to support overlapping areas. For using another 
...
def _get_map_from_db(config: SimpleNamespace, target_area_id: int) -> Tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
    nodes = get_map_nodes_from_db(target_area_id, config.schema)
    logging.info(f"{len(nodes)} nodes fetched from db")
    edges = get_map_edges_from_db(config.srid, target_area_id, config.schema)
...
def export(config: Dict, target_area_id: int):
    """
    ...
    :param target_area_id: id of the area for which the map is generated
    ...
    # download and process map
        ...
        nodes, edges = _get_map_from_db(config, target_area_id)
