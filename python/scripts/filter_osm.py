...
def load_multipolygon_by_id(relation_id: str | int):
...
""",
    ...
    parser.add_argument(
        ...
        nargs="?",
        help="Path to expression file for filtering tags (required for 'f' tag)",
    )
    parser.add_argument(
        "-c",
        dest="coords",
        nargs="?",
        help="Bounding box coordinates or path to config file (required for 'b' tag)",
    )
    parser.add_argument(
        "-rid", dest="relation_id", nargs="?", help="relation ID (required for 'b' tag)"
    )
    parser.add_argument(
        "-s", dest="strategy", help="Strategy type (optional for 'id', 'b' tags)"
    )
    parser.add_argument(
        "-R",
        dest="omit_referenced",
        action="store_true",
        help="Omit referenced objects (optional for 'f' tag)",
    )
        ...
        case "b":
            ...
            if not args.coords:
                raise MissingInputError(
                    "Coordinates or config file need to be specified with the 'b' tag."
                )
