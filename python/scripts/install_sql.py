...
logger = LOGGER.get_logger("install_sql")
...
logger.info("Checking availability of extensions in the database")
...
for extension_name, status in db.execute_sql_and_fetch_all_rows(sql):
    logger.info(f"{extension_name}: {status}")
...
if extensions["pgtap"] and not db.execute_sql_and_fetch_all_rows(sql):
    logger.info("Enabling pgtap")
    execute_sql_file(sql_dir / "testing_extension.sql", multistatement=True)
else:
    if not extensions["pgtap"]:
        logger.warning("pgtap extension is not available")
    else:
        logger.info("pgtap extension is already enabled")
