...
def execute_sql_file(sql_file: Path, multistatement: bool = False) -> int:
    logging.info(f"Executing {sql_file}")
    retcode = 0
    if multistatement:
        retcode = db.execute_script(sql_file)
    else:
...
def main():
    ...
    if (
        not extensions["postgis"]
        or not extensions["pgrouting"]
        or not extensions["hstore"]
    ):
