...
def run_osm2pgsql_cmd(
    ...
    if not force and not check_empty_or_nonexistent_tables(schema, config):
        raise TableNotEmptyError(
            "Attempt to overwrite non-empty tables. Use '--force' flag to proceed."
        )
...
def post_process_osm_import(style_filename: str, schema: str | None = None) -> int:
    ...
    command = [
        ...
        CREDENTIALS.host,
        "-p",
        str(CREDENTIALS.db_server_port),
    ]
    if schema:
...
def import_osm_to_db(
    input_file: str, force: bool, style_file_path: str = None, schema: str = "public"
) -> int:
...
b  : Extract greatest bounding box from given relation ID of 
    ...
    parser.add_argument(
        "-id", dest="relation_id", help="Relation ID (required for 'b' flag)"
    )
    parser.add_argument(
        "-l",
        dest="style_file",
        nargs="?",
        default="resources/lua_styles/default.lua",
        help="Path to style file (optional for 'b', 'u' flag)",
    )
    parser.add_argument(
        "-o",
        dest="output_file",
        help="Path to output file (required for 's', 'r', 'sr' flag)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        ...
        dest="schema",
        default="public",
        help="Database schema (for 'b', 'u' flag)",
    )
    parser.add_argument(
        "--force",
        dest="force",
        action="store_true",
        help="Force overwrite of data in existing tables in schema (for 'b', 'u' flag)",
    )
...
def main(arg_list: list[str] | None = None):
    ...
    match args.flag:
        ...
        case "u":
            ...
            run_osm2pgsql_cmd(
                CREDENTIALS, args.input_file, args.style_file, args.schema, args.force
            )
        case "b":
