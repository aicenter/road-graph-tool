...
from scripts.filter_osm import (InvalidInputError, MissingInputError,
                                check_strategy, extract_bbox, extract_id,
...
def test_is_valid_extension_valid():
    valid_file = "test.osm"
    assert is_valid_extension(valid_file)


def test_is_valid_extension_invalid():
    invalid_file = "test.pdf"
    assert not is_valid_extension(invalid_file)
...
def test_check_strategy():
    ...
    with pytest.raises(
        InvalidInputError,
        match="Invalid strategy type. Call filter_osm.py -h/--help to display help.",
    ):
...
def test_load_multipolygon_by_id_contains_id(mocker, expected_multipolygon_id):
    ...
    ), f"Result does not contain the relation_id {relation_id}"
...
def test_extract_id_contains_id():
    ...
    with open(output_file, "rb") as f:
        ...
        assert (
            contains_relation_id
        ), f"File content does not contain the relation_id {relation_id_str}."
...
def test_extract_bbox_config_valid(mock_subprocess_run, mock_os_path_isfile):
    ...
    extract_bbox(input_file, coords)
    expected_command = ["osmium", "extract", "-c", coords, input_file]
    mock_subprocess_run.assert_called_once_with(expected_command)


def test_extract_bbox_coords_and_config_invalid(
    mock_subprocess_run, mock_os_path_isfile
):
...
def test_main_b_missing_coord():
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        with pytest.raises(
            MissingInputError,
            match="Coordinates or config file need to be specified with the 'b' flag.",
        ):
            main(arg_list)
...
def test_main_b_config_valid(mocker):
    with tempfile.NamedTemporaryFile(
        suffix=".osm"
    ) as tmp_file, tempfile.NamedTemporaryFile(suffix=".json") as config_file:
...
def test_main_f_missing():
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        with pytest.raises(
            MissingInputError, match="Expression file needs to be specified."
        ):
            main(arg_list)
...
def test_main_f_expressionfile_invalid():
    ...
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        with pytest.raises(
            FileNotFoundError, match=f"File '{invalid_expression}' does not exist."
        ):
            main(arg_list)
...
def test_main_f_valid(mocker):
    with tempfile.NamedTemporaryFile(
        suffix=".osm"
    ) as tmp_file, tempfile.NamedTemporaryFile(suffix=".txt") as expression_file:
