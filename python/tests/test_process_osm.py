...
from scripts.process_osm import (import_osm_to_db, main, run_osm2pgsql_cmd,
                                 run_osmium_cmd)
...
def test_run_osmium_cmd_sort_renumber(mocker):
    ...
    mock_subprocess_run.side_effect = [
        subprocess.CompletedProcess(args=[], returncode=0),  # for sort
...
def test_import_to_db_valid(mocker):
    mocker.patch(
        "scripts.process_osm.os.path.exists",
        side_effect=lambda path: path
        in ["resources/to_import.osm", "resources/lua_styles/default.lua"],
    )
    ...
    mock_run_osm2pgsql_cmd.assert_called_once_with(
        config,
        "resources/to_import.osm",
        "resources/lua_styles/default.lua",
        "osm_testing",
        True,
    )
    assert file_size == 1
...
def test_import_to_db_invalid_file(mocker):
    mocker.patch(
        "scripts.process_osm.os.path.exists",
        side_effect=lambda path: path == "resources/lua_styles/default.lua",
    )
...
def test_main_invalid_inputfile():
    ...
    with pytest.raises(
        FileNotFoundError, match="File 'invalid_file.osm' does not exist."
    ):
        main(arg_list)
...
def test_main_srsr_invalid(test_input):
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        arg_list = [test_input, tmp_file.name]
        with pytest.raises(
            MissingInputError, match="An output file must be specified with '-o' flag."
        ):
            main(arg_list)
...
def test_main_srsr_valid(mocker, test_input):
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        mock_run_osmium_cmd = mocker.patch("scripts.process_osm.run_osmium_cmd")
        main(arg_list)
        mock_run_osmium_cmd.assert_called_once_with(
            arg_list[0], arg_list[1], arg_list[3]
        )
...
def test_main_default_style_valid(mocker):
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        main(arg_list)
        mock_run_osm2pgsql_cmd.assert_called_once_with(
            config, arg_list[1], "resources/lua_styles/default.lua", "public", False
        )
...
def test_main_input_style_valid(mocker):
    with tempfile.NamedTemporaryFile(
        suffix=".osm"
    ) as tmp_input, tempfile.NamedTemporaryFile(suffix=".lua") as tmp_lua:
        arg_list = ["u", tmp_input.name, "-l", tmp_lua.name]
        mock_run_osm2pgsql_cmd = mocker.patch("scripts.process_osm.run_osm2pgsql_cmd")
        main(arg_list)
        mock_run_osm2pgsql_cmd.assert_called_once_with(
            config, arg_list[1], arg_list[3], "public", False
        )
...
def test_main_style_file_invalid():
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        with pytest.raises(
            FileNotFoundError, match="File 'invalid_style.lua' does not exist."
        ):
            main(arg_list)
...
def test_main_style_file_extension_invalid():
    with tempfile.NamedTemporaryFile(
        suffix=".osm"
    ) as tmp_file, tempfile.NamedTemporaryFile(suffix=".txt") as invalid_lua:
        arg_list = ["u", tmp_file.name, "-l", invalid_lua.name]
        with pytest.raises(
            InvalidInputError, match="File must have the '.lua' extension."
        ):
            main(arg_list)
...
def test_main_bbox_valid(mocker):
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        mock_extract_bbox = mocker.patch(
            "scripts.process_osm.extract_bbox", return_value=(10, 20, 30, 40)
        )
        mock_run_osm2pgsql_cmd = mocker.patch("scripts.process_osm.run_osm2pgsql_cmd")
        main(arg_list)
...
def test_main_bbox_id_missing():
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        with pytest.raises(
            MissingInputError, match="Existing relation ID must be specified."
        ):
            main(arg_list)
