...
from scripts.process_osm import (RESOURCES_DIR, STYLES_DIR, import_osm_to_db,
                                 main, run_osmium_cmd)
...
def test_import_to_db_valid(mocker):
    mocker.patch(
        "os.path.exists", side_effect=lambda path: path.endswith("to_import.osm")
    )
...
def test_main_srsr_invalid(test_input):
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        arg_list = [test_input, tmp_file.name]
        with pytest.raises(
            MissingInputError, match="An output file must be specified with '-o' tag."
        ):
            main(arg_list)
...
def test_main_default_style_valid(mocker):
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        main(arg_list)
        mock_run_osm2pgsql_cmd.assert_called_once_with(
            config, arg_list[1], str(STYLES_DIR / "default.lua")
        )
...
def test_main_bbox_valid(mocker):
    with tempfile.NamedTemporaryFile(suffix=".osm") as tmp_file:
        ...
        mock_run_osm2pgsql_cmd.assert_called_once_with(
            config, arg_list[1], str(STYLES_DIR / "default.lua"), "10,20,30,40"
        )
