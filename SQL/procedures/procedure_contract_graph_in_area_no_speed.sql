CREATE OR REPLACE PROCEDURE contract_graph_in_area_no_speed(IN target_area_id smallint, IN target_area_srid integer)
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE TEMPORARY TABLE road_segments AS (
	SELECT * FROM select_node_segments_in_area_no_speed(target_area_id, target_area_srid)
);
